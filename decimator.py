...
class Decimator:
    ...
    def __init__(self, config: PulseDetectConfig, output_port: int = None, stages: tuple = (8, 8, 6)):
        """
    ...
    def decimate_multistage(self, samples: np.ndarray) -> np.ndarray:
        ...
        for stage, factor in enumerate(self.stages, 1):
            try:
                # Use resample_poly for decimation with built-in anti-aliasing filter
                # resample_poly(x, up, down) resamples by up/down ratio
                # For decimation, up=1, down=factor
                result = signal.resample_poly(result, 1, factor, window='hamming')

                # Check for non-finite values after resampling
                if not np.all(np.isfinite(result)):
                    print(f"⚠️  Stage {stage}: Non-finite values after resampling! Sanitizing...")
            ...
            while self.running:
                ...
                if self.input_sequence is not None:
                    ...
                    if input_seq != expected:
                        ...
                        print(f"⚠️  INPUT: Dropped {dropped} messages! "
...
def main():
    ...
    decimator = Decimator(config, args.output_port, tuple(args.stages))
